...
try:
    ...
    if sys.platform.find('win') == -1:
        ...
        pdcurses = "pdc34dllu/pdcurses.dll"
...
def initscr():
    ...
    if NCURSES:
        ...
        tmpstr = ctypes.c_void_p(pdlib.initscr())
        if tmpstr.value:
            stdscr = tmpstr
        return stdscr
...
def setupterm(term, fd, **kwargs):
    if NCURSES:
        try:
            return curses.setupterm(term, fd)
        except curses.error:
            return ERR
    else:
        return pdlib.setupterm(term, fd, None)
